  ...
  if (event.type == SDL_KEYDOWN) {      
    if (!started) {
      ...
      if (event.key.keysym.sym == SDLK_LEFT || event.key.keysym.sym == SDLK_a) {
        ...
        m_trackBallModel.mouseMove(mousePosition, m_panSpeed);
        m_trackBallLight.mouseMove(mousePosition, m_panSpeed);
...
void OpenGLWindow::initializeGL() {
  ...
  // Load default model
  loadModel(getAssetsPath() + "torre.obj");
...
void OpenGLWindow::loadModel(std::string_view path) {
  m_model.loadDiffuseTexture(getAssetsPath() + "maps/Bricks26_col.jpg");
...
//  m_trackBallModel.mouseMove(mousePosition, m_panSpeed * deltaTime);
//  m_trackBallLight.mouseMove(mousePosition, m_panSpeed * deltaTime);
